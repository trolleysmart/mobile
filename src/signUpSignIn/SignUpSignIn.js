...
class UserSignInSignUpPresentational extends Component {
  ...
  onSignInEmailAddressChanged = text => {
    this.setState({
      data: this.state.data
        .set('signInEmailAddress', text.toLowerCase())
  ...
  onSignUpEmailAddressChanged = text => {
    this.setState({
      data: this.state.data
        .set('signUpEmailAddress', text.toLowerCase())
  ...
  onSignUpConfirmPasswordChanged = text => {
    this.setState({
      data: this.state.data
        .set('signUpConfirmPassword', text)
  ...
  onSignInClicked = () => {
    if (this.state.data.get('signInInputAreaHidden')) {
      this.setState({
        data: this.state.data
          .set('signInInputAreaHidden', false)
      ...
      if (
        !this.getSignInEmailErrorMessage(true) &&
        !this.getSignInPasswordErrorMessage(true)
      ) {
        this.props.onSignInClicked(
          this.state.data.get('signInEmailAddress'),
          this.state.data.get('signInPassword'),
        );
      }
    }
  };
  ...
  onSignUpClicked = () => {
    if (this.state.data.get('signUpInputAreaHidden')) {
      this.setState({
        data: this.state.data
          .set('signInInputAreaHidden', true)
      ...
      if (
        !this.getSignUpEmailErrorMessage(true) &&
        !this.getSignUpPasswordErrorMessage(true) &&
        !this.getSignUpConfirmPasswordErrorMessage(true)
      ) {
        this.props.onSignUpClicked(
          this.state.data.get('signUpEmailAddress'),
          this.state.data.get('signUpPassword'),
        );
      }
    }
  };
  ...
  getSignInPasswordErrorMessage = buttonPressed => {
    if (this.state.data.get('signInPasswordChanged') || buttonPressed) {
      return this.state.data.get('signInPassword')
        ? null
    ...
    return this.state.data.get('signInInputAreaHidden')
      ...
      : <View>
          ...
          {this.renderErrorMessage(
            this.getSignInEmailErrorMessage(
              this.state.data.get('signInButtonPressed'),
            ),
          )}
          ...
          />
          {this.renderErrorMessage(
            this.getSignInPasswordErrorMessage(
              this.state.data.get('signInButtonPressed'),
            ),
          )}
          {this.renderSignInButton()}
  ...
  renderSignUpInputArea = () => {
    return this.state.data.get('signUpInputAreaHidden')
      ...
      : <View>
          ...
          />
          {this.renderErrorMessage(
            this.getSignUpEmailErrorMessage(
              this.state.data.get('signUpButtonPressed'),
            ),
          )}
          <FormLabel>Password</FormLabel>
          <FormInput
          ...
          />
          {this.renderErrorMessage(
            this.getSignUpPasswordErrorMessage(
              this.state.data.get('signUpButtonPressed'),
            ),
          )}
          ...
          />
          {this.renderErrorMessage(
            this.getSignUpConfirmPasswordErrorMessage(
              this.state.data.get('signUpButtonPressed'),
            ),
          )}
          {this.renderSignUpButton()}
    ...
    if (this.props.signUpOrSignInIsInProgress) {
      return (
        <Row>
          <Col>
            <ActivityIndicator
              animating={this.props.signUpOrSignInIsInProgress}
              size="large"
              color="#3b5998"
    ...
    return (
      <ScrollView style={Styles.scrollView} keyboardShouldPersistTaps="always">
        <Grid style={Styles.grid}>
          ...
          <Row>
            <Col>
              {this.renderSignInInputArea()}
            </Col>
          </Row>
          <Row>
            <Col>
              {this.renderSignUpInputArea()}
            </Col>
          </Row>
        </Grid>
      </ScrollView>
