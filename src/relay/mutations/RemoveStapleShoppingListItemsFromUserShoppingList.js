...
const mutation = graphql`
  mutation RemoveStapleShoppingListItemsFromUserShoppingListMutation($input: RemoveStapleShoppingListItemsFromUserShoppingListInput!) {
    removeStapleShoppingListItemsFromUserShoppingList(input: $input) {
      errorMessage
    }
  }
`;
...
function commit(environment, userId, shoppingListId, stapleShoppingListItemId) {
  return commitMutation(environment, {
    ...
    updater: store => {
      const payload = store.getRootField('removeStapleShoppingListItemsFromUserShoppingList');
